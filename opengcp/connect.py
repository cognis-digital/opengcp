"""Native cognis-connect emit for opengcp — forward findings to any platform.

Maps opengcp's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    opengcp ... --format json | opengcp-emit --to stix
    opengcp-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('source', None)
        out['title'] = rec.get('title', 'No Title')
